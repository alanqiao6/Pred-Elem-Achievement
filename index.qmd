...
merged_data <- merged_data |> 
  filter(!is.na(MN)) |>
  filter(!is.na(family_poverty)) |>
  filter(!is.na(percentage_single_mother)) |>
  filter(!is.na(ratio_of_one_to_both)) |>
  filter(!is.na(average_center_child_care)) |>
  filter(!is.na(average_family_child_care)) |>
  mutate(
   MN = as.numeric(MN)) 

maximum_regression_model <- linear_reg() |>
  fit(MN ~ family_poverty + percentage_single_mother + average_center_child_care, data = merged_data)
```
