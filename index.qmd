...
ggplot(us_states_expenditures, aes(x = long, y = lat, group = group, fill = inf_adj_perchild)) +
  geom_polygon() +
  theme_void() + 
  scale_fill_gradient(low = "gray90", high = "navyblue") +
...
maximum_regression_model <- linear_reg() |>
  fit(MN ~ family_poverty + percentage_single_mother + average_center_child_care + average_family_child_care + one_parent_households + ratio_of_one_to_both, data = merged_data)
```
